...
export async function translateMarkdown(
  ...
  for (const segment of segments) {
    ...
    for (const chunk of splitTextForTranslation(segment.content, maxChunkChars)) {
      ...
      const translatedChunk = await options.translateChunk(protectedChunk.text, options.targetLanguage);
      output.push(restoreInlineMarkdown(translatedChunk, protectedChunk.tokens));
    }
  }
